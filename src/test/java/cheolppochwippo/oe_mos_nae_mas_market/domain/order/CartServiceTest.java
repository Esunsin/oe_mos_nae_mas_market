...
public class CartServiceTest {
	...
	@BeforeEach
	void before() {
		cartService = new CartServiceImpl(orderRepository, productRepository, messageSource);
	}
	...
	void createOrder() {
		//given
		Product product = new Product(100L, "productA", "best Product", 12000L, 10000L, 2000L, 10L,
		...
		//when
		SingleOrderInCartResponse orderInCart = cartService.createOrderInCart(customer, quantity,
			product.getId());
		//then
	...
	void createSameOrder() {
		...
		given(productRepository.findById(product.getId())).willReturn(Optional.of(product));
		given(orderRepository.findOrderByProductIdAndUserBeforeBuy(customer,
			product.getId())).willReturn(Optional.of(order));
	...
	void updateQuantity() {
		...
		SingleOrderInCartResponse singleOrderInCartResponse = cartService.updateQuantity(
			updateQuantity, 100L);
	...
	void updateLessQuantity() {
		...
		//then
		Assertions.assertThrows(MinimumQuantityException.class,
			() -> cartService.updateQuantity(updateQuantity, 100L));
	}
